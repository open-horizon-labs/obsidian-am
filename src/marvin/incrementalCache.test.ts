...
describe("incremental Marvin cache", () => {
	...
	it("resumes an opaque checkpoint, coalesces concurrent polls, and persists it", async () => {
		...
		// An empty/unchanged page must not force a full cache-file rewrite —
		// verified against a real ~190KB cache file that this was happening
		// on every idle poll tick. The advanced checkpoint still lives in
		// memory for the next call's `since`; it doesn't need to hit disk
		// until something real changes or the idle-flush interval elapses.
		expect(store.saves).toHaveLength(0);
	});

	it("persists immediately once a poll actually changes something", async () => {
		const store = new MemoryStore();
		store.value = state();
		const changes = new QueuedChanges([
			{
				results: [{
					id: "task",
					seq: { seq: 2 },
					doc: { _id: "task", db: "Tasks", title: "Draft", parentId: "project", done: true },
				}],
				lastSeq: { seq: 2 },
				pending: 0,
			},
		...
		const cache = new IncrementalMarvinCache({
			sourceKey: "source",
			changes,
			snapshot: { getCategories: async () => [], getChildren: async () => [] },
			store,
			now: () => 2_000,
		});
	...
	it("persists an idle checkpoint periodically, bounding replay on restart without rewriting on every tick", async () => {
		const store = new MemoryStore();
		store.value = state();
		let clock = 2_000;
		const changes = {
			changes: vi.fn(async (): Promise<CouchChangesPage> => ({
				results: [],
				lastSeq: { seq: 2 },
				pending: 0,
			})),
		...
		const cache = new IncrementalMarvinCache({
			...
			store,
			now: () => clock,
			checkpointPersistIntervalMs: 1_000,
		});

		await cache.sync();
		expect(store.saves).toHaveLength(1); // never persisted before — first tick always flushes once

		clock += 500;
		await cache.sync();
		expect(store.saves).toHaveLength(1); // still within the idle-flush interval, no rewrite

		clock += 600;
		await cache.sync();
		expect(store.saves).toHaveLength(2); // interval elapsed with no real change — flush anyway
		expect(store.saves.at(-1)?.lastSeq).toEqual({ seq: 2 });
	});
