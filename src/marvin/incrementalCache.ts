import type { Task } from "@open-horizon/marvin-client";
import {
	categoriesFromState,
	childrenFromState,
...
export type { CachedMarvinContainer, CachedMarvinItem, IncrementalCacheState };
...
export class IncrementalMarvinCache {
	...
	constructor(private readonly options: {
		...
		// A no-op sync still rewrites the entire persisted cache file unless
		// bounded: verified against a real ~190KB cache file that every idle
		// poll tick was rewriting it just to persist an advanced checkpoint,
		// with no data change at all. Real changes still persist immediately;
		// idle polling persists at most this often.
		checkpointPersistIntervalMs?: number;
	}) {}
	...
	getCategories(): CachedMarvinContainer[] | undefined {
		return this.state ? categoriesFromState(this.state) : undefined;
	}

	getChildren(parentId: string): CachedMarvinItem[] | undefined {
		return this.state ? childrenFromState(this.state, parentId) : undefined;
	}
	...
	private async ensureHydrated(): Promise<IncrementalUpdate | undefined> {
		...
		const stored = parseIncrementalCacheState(
	...
	private async pullChanges(
		...
		for (let pageNumber = 0; pageNumber < maxPages; pageNumber += 1) {
			...
			const now = this.options.now?.() ?? Date.now();
			this.state.lastSuccessfulSyncAt = now;
			mergeApplied(combined, applied);

			const isFinalPage = (page.pending ?? 0) <= 0 && page.results.length < 500;
			const persistIntervalMs = this.options.checkpointPersistIntervalMs ?? 60_000;
			const checkpointIsStale = now - this.lastPersistedAt >= persistIntervalMs;
			if (applied.changed || (isFinalPage && checkpointIsStale)) {
				await this.options.store.save(this.state);
...
export function categoryFromDocument(
...
export function isPresentDocument(doc: MarvinDatabaseDocument): boolean {
